...
class MyHelper
{
	...
	public function Check_Gaz($test, $date, $day)
	{
		$form = 1;

		$startday = $this->params['gaz_startDay'];
		$stopday = $this->params['gaz_stopDay'];

		$date_elements  = explode("-",$date);
		$datem = $date_elements[0]. "-". $date_elements[1];

		// Условие доступности формы для ввода показаний счетчиков газа
		// $form == 0 - форма не доступна
		if($startday < $stopday){
			if( ($day < $startday) || ($day > $stopday) ) $form = 0;
		}
	...
	public function Check_Electro($test, $date, $day)
	{
		$form = 1;

		$startday = $this->params['electro_startDay'];
		$stopday = $this->params['electro_stopDay'];

		$date_elements  = explode("-",$date);
		$datem = $date_elements[0]. "-". $date_elements[1];

		// Условие доступности формы для ввода показаний счетчиков электроэнергии
		// $form == 0 - форма не доступна
		if($startday < $stopday){
			if( ($day < $startday) || ($day > $stopday) ) $form = 0;
		}
