...
<tr>
	...
	$delta = abs($this->params['electro_startDay'] - $this->params['electro_stopDay']);
	if( ($delta > 1 ) && ($delta < 30) )
	{
		//echo abs($this->params['electro_startDay'] - $this->params['electro_stopDay']);
		echo '<h3>Внимание!!! Вы можете ввести показания счетчиков только c '.
		$this->params['electro_startDay'] .' по ' . $this->params['electro_stopDay'] .
	...
	?>
			...
			<fieldset>
				...
				for($i = 1; $i <= $this->dataofsn->counts; $i++)
				{
					...
					<tr>
						...
						<td align=left><?php echo $this->form->getInput('electro'.$i); ?><FONT
					...
					<tr>
						<td align=left><input type="hidden" name="option" value="com_tsj" />
							<input type="hidden" name="task" value="electros.submit" /> <!--<button class="back_button" id="submit" name="submit" type="submit" value="Передать показания"></button>-->
		...
		$linksn = $this->params['electro_linksn'];
		if(!empty($linksn))
		{
