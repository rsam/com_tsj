...
<tr>
	...
	if(!empty($this->dataofcounter))
	{
		...
		$count_rows = 0;
		if($this->params['electro_show_rows_sn'] == '1') $count_rows++;
		if($this->params['electro_show_rows_electro'] == '1') $count_rows++;
			
		// Нужно ли отображать серийные номера
		if ($this->params['electro_show_rows_sn']) $show_sn = 1;
		else $show_sn = 0;

		// Нужно ли отображать electro
		if ($this->params['electro_show_rows_electro']) $show_delta = 1;
		else $show_delta = 0;
		...
		for($i = 1; $i <= $this->dataofsn->counts; $i++)
		{
			...
			echo ' ' . $this->dataofsn->{'electro_name_'.$i} . '</th>';
		}
		...
		foreach ( $this->dataofcounter as $row )
		{
			...
			for($i = 1; $i <= $this->dataofsn->counts; $i++)
			{
				...
				$rashod_sum += $rashod*1; // electro суммарно
				if($rashod < 0){
				...
				if($show_delta){
					if($rashod < 0){
						...
						echo '<td align="center">' . $rashod . '</td>'; // electro
					}
				}
					
			} // for
			echo '<td align="center">' . $rashod_sum . '</td>'; // electro холодная суммарно
			echo '</tr>';
